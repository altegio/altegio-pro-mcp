...
- **Plumbing, not a live boundary.** The scope names are PLACEHOLDERS: the shape follows the ratified v3 convention (`domain:action`) and most names are verbatim from `../biz.erp/docs/research/api-standardization/v3-plan/scope-catalog.md`, which is explicitly not final and owned by the API team. `analytics:*` and `api:read` had to be invented — no v3 domain covers reports, and the universal executor reaches every documented GET under one tool name.
...
- **Safe no-op today:** a caller that declares no scopes — the public HTTP endpoint, stdio, the closed Google deployment, all of them — passes straight through. A malformed header counts as "not declared" rather than "empty grant", so a proxy bug cannot refuse every call. Scope tokens are validated against the RFC 6749 grammar on the way in, so echoing them into a refusal cannot smuggle markup into model context.
- **Implication:** only `X:write ⊇ X:read` on the same domain (v3 A8/F-13). `create`, `manage_access`, `capture`, `refund` are never implied; `chain_*` never satisfies a location scope. `clients:read_contact` is a field gate V1 cannot express and is deliberately not faked.
