...
**Onboarding structured results (2026-09-24)**
- **The defect:** 11 of the 12 onboarding tools declare an `outputSchema` (all but `onboarding_rollback_phase`) yet returned text only. `@modelcontextprotocol/sdk` `Client.callTool` throws for a successful result without `structuredContent` once `listTools()` has cached the schemas, so SDK-based hosts could not use any successful wizard step. Every success path now returns it; `isError` results stay text-only.
- **Schemas fixed, not just satisfied** (`src/tools/output-schemas.ts`): `onboardingStatusOutput.phase` was a number; status is now `{location_id, phase, completed, started_at, updated_at, checkpoints[{phase, entity_count, completed_at}], total_entities}`, batch steps `{location_id, phase, created, failed, created_ids, errors}`, preview `{total, fields, preview}` — all fields required. Phases use agent-facing names (`toAgentPhase`, now in `src/types/onboarding.types.ts`); `test_bookings` never leaves the state file.
- **IDs for the next step:** staff, category and service steps list created IDs in the text (positions already did); every step returns `created_ids` (for schedules: the team member IDs). Client IDs stay out of the text.
- **Free text:** a failed row's name and the API reason are sanitized separately, then joined with ` — ` (a colon would let a row named "User" read as a turn marker) — joined first, a reason opening with "System:" survived mid-line in the fence. Structured `errors` equal the fenced lines. Test appointments now report refused bookings (`onboarding_create_test_appointments` moved to `FREE_TEXT` in `untrusted-coverage.test.ts`). Preview rows are `sanitizeUntrustedDeep`-ed; a non-object first entry is an empty preview.
- **Tests:** `src/__tests__/onboarding-structured-output-e2e.test.ts` (real SDK `Client` ↔ `Server`, 2020-12 re-validation, fails when an onboarding tool is not walked; `HOME` redirected to a temp dir because the state manager writes under it) and canary/shape tests in `src/tools/__tests__/onboarding-handlers.test.ts`.
