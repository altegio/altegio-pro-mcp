...
Treat the anchor date as part of the dense window. Do not silently exclude today or assume Sunday is closed: closures belong in the location profile, and every other date must meet the same schedule and appointment minima. A location described as open today must have both a working schedule and visible appointments today.
...
The first command is an audit-only pass. `--apply` updates identities, curated staff/services/links, schedules from 60 days before the anchor through 60 days after it, booking settings/forms, clients, a dense ±45-day appointment plan including today when the location is open, and payments for arrived demo visits. The plan is built day by day, packs services inside each work shift, and enforces location-specific minimum staff and appointment counts for every open day. Before each write it re-reads that employee-day, confirms the interval is inside a working schedule, and rejects a live overlap. For a future appointment it additionally asks the documented `book_times` API for the exact service-specific slot and, when public sessions are exposed, requires a successful `book_check` immediately before the administrative create. Some demo locations have a usable B2B calendar but expose no public online-booking sessions; report that fallback explicitly and rely on the just-in-time B2B checks plus the create endpoint's default `save_if_busy=false` enforcement. Elapsed historical fixtures cannot use public online-booking availability and are explicitly limited to schedule plus live-occupancy validation. Every successful create must appear in an immediate day read-back. Exact names and staff+datetime intervals make reruns idempotent within the same anchor date. Set `DEMO_ANCHOR_DATE=YYYY-MM-DD` for a reproducible historical run. The helper opens a separate scoped hosted-MCP session per location, opens a fresh session for the final audit, paginates reads beyond 300 appointments, paces writes, retries bounded `429` responses, and never prints tokens.

## Availability and calendar visibility

Do not equate `has_schedule`, a working day, and a free time slot:

1. `is_bookable=true` and `has_schedule=true` only describe the provider globally.
2. `get_schedule` must show `is_working=true` for the exact employee-date and the complete appointment interval must fit inside one returned working slot.
3. A fresh day-level `get_appointments` read must show no overlap. Never rely only on a window snapshot made earlier in the run.
4. For current or future online-bookable time, ask documented `GET /book_times/{location_id}/{team_member_id}/{date}` for the exact service-specific session. When it exposes sessions, require the exact selected time, immediately call documented `POST /book_check/{location_id}`, and require HTTP 201. The backend calculation is authoritative because it also applies service, resource, booking-window, and other dynamic constraints.
5. If `book_times` returns no sessions for a demo location whose B2B calendar is operational, report `online_unavailable_fallbacks` rather than claiming that online availability was validated. A future administrative fixture may then proceed only after the exact schedule and just-in-time occupancy checks pass; leave `save_if_busy` absent/false so the B2B create endpoint performs its own final conflict check. A non-empty session list that omits the desired time is a real conflict and must not fall back.
6. For elapsed historical demo fixtures, `book_times`/`book_check` intentionally cannot prove availability. Create only when the authoritative B2B schedule and a just-in-time appointment read both pass, and report this as historical administrative validation rather than online availability.
7. After every create, re-read the employee-day and require the returned appointment ID to be visible.

During the final audit, join every curated appointment to its exact employee-date schedule. Fail if the date is non-working/missing or if the inferred service duration extends outside all working slots. Report these separately as `nonworking_or_missing_day` and `outside_working_slots`; they are the likely cause when an appointment exists in API data but does not appear correctly in the calendar. For demo-owned appointments, repair the exact employee-day by merging the appointment interval into that day's existing slots, read the schedule back, and re-run the complete visibility audit. Do not broaden every provider's regular weekly hours to conceal a small number of bad dates.
