...
import { getCustomerCashReceipts } from '../cash-receipts.js';
...
describe('customer cash receipts', () => {
  it('reconciles components while separating custom income from customer cash', async () => {
    const result = await getCustomerCashReceipts(client, {
      location_id: 4564,
  ...
  it.each([
    ...
    'rejects a partial or oversized month window',
    async (date_from, date_to) => {
      await expect(
        getCustomerCashReceipts(client, {
          location_id: 4564,
          date_from,
          date_to,
        })
