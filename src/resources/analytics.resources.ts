...
export function renderCoverage(): string {
  ...
  return [
    ...
    '- Revenue, average check and visits by client (contacts opt-in).',
    '- Client retention by team member.',
    '- Per-client forecast factors from the stable export (contacts opt-in).',
    '- Revenue, costs and profit by service or service category.',
    '- Sales and working-hour efficiency by team member.',
    '- Working, booked and idle hours by team member with capacity totals.',
    ...
    '- A posted operating-ledger statement with sales-stream memo figures, service contribution and explicit cost-coverage limits.',
    '- Capacity heatmaps by hour, weekday or date-hour using scheduled time as the denominator.',
    '- Revenue-leakage signals for no-shows, cancellations, unpaid-risk flags, discounts and optional unbooked capacity.',
    '- A team-member × service matrix with contribution, shares and minimum-sample ranking safeguards.',
    '- Inventory velocity, days of cover, reorder risk and deterministic reorder quantities.',
    '',
    ...
    '- There is no ad-hoc report builder. Use the curated report tools for their stable tables; arbitrary custom tables and a complete statutory cash-flow statement remain unavailable.',
