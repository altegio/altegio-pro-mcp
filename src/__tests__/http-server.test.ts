...
describe('HTTP server per-request identity', () => {
  ...
  const fakeTransport = (sink: (id: Captured) => void): SSEServerTransport =>
    ({
      handlePostMessage: async (_req: unknown, res: { status: (n: number) => { json: (b: unknown) => void } }) => {
  ...
  const post = (port: number, sessionId: string, headers: Record<string, string>) =>
    fetch(`http://127.0.0.1:${port}/mcp?sessionId=${sessionId}`, {
      method: 'POST',
