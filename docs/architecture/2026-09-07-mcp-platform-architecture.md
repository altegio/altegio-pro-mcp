...
**Addendum (2026-09-17) — the surface table: the same reviewability rule, applied to the *views*.** The catalog makes `tools/list` reviewable for one view. It says nothing about the question that actually grew hard: eight mechanisms now decide whether a tool reaches a given address and whether the call it receives runs there — the report-builder closure (`DISABLED_TOOL_NAMES`), the default view's excluded prefixes, excluded names and re-admitted names, the password-login switch, the read-only rule, the token-scope map (D6 addendum) and the human-confirmation gate (D8 addendum). Each is justified on its own terms; collapsing them into one list would destroy the justifications, which is worse than six lists. What was missing was not fewer mechanisms but **one place that joins them**.

`src/tools/surface.ts` produces one cell per tool × per view: served or withheld, a *machine value* for the reason, and the gates that still refuse the call. It is not a second implementation — membership comes from `decideView` in `src/tools/facets.ts`, the only membership function in the server, and `buildFacetIndex` is a projection of the same function, pinned by a test that compares them under every switch combination. `src/tools/inventory.ts` is the single enumeration of what exists, disabled tools included, because "why is this tool nowhere?" cannot be answered about a tool the inventory has already dropped.

The table renders to the committed `docs/architecture/tool-surface.md` (`npm run surface:build`, checked by `npm run surface:check` and by the test suite), so a surface change reaches a reviewer's diff exactly the way a catalog change does. Tool counts follow the same rule: `README.md` and `CLAUDE.md` quote a computed sentence and per-category counts that a test pins — CLAUDE.md had claimed "69 total" while 72 were defined and 66 served.
...
**Addendum (2026-09-17) — token scopes are enforced on execution, and only on execution.** The v3 authorization RFC settled that the access boundary is the *scope of the token*: the MCP spec forbids clients from treating annotations as a security decision, and a separate address (D3 addendum) only lets a deployment pick a rights profile — it restricts nothing by itself. The OAuth proxy has forwarded the caller's granted scopes as `x-mcp-auth-scope` since the platform shipped and nothing read them; now every tool declares what its execution requires and the `tools/call` handler checks it.

- **One map file.** `src/tools/scopes.ts` holds the vocabulary, the tool→scope map, the satisfaction rule and the refusal text. No tool definition names a scope — `defineTool` fills `ToolDefinition.requiredScopes` from the map by tool name — so approving a different vocabulary is one edit rather than seventy, and a coverage test fails the build when a tool has no entry.
- **Two vocabularies, one rule, one file.** Requirements are written in the v3 `domain:action` names — their shape follows the ratified convention and most are verbatim from the v3 P0 catalog, which is explicitly not final and is owned by the API team (`analytics:*` and `api:read` had to be invented: no v3 domain covers reports, and the universal executor reaches every documented GET under one tool name). The grants that actually arrive are the platform's coarse `mcp:pro:read` / `mcp:pro:write`, which the proxy issues per route. Both live in `src/tools/scopes.ts` and the reconciliation rule is stated once, in `scopeSatisfied`; the platform pair is deleted the day v3 issues `domain:action` tokens.
- **Execution only, never `tools/list` (D7).** One path serves one tool list to every connection, whatever the caller's grant. A tool a caller cannot run is still listed and explains itself when called. Filtering the list by scope would make `tools/list` vary per connection, which 2026-07-28 forbids, and would also make the surface unreviewable in a diff (D4).
- **Only `X:write ⊇ X:read` is implied** (v3 A8/F-13), on the same domain. `create`, `manage_access`, `capture` and `refund` always need an explicit grant, and `chain_*` never satisfies a location scope. `clients:read_contact` gates *fields* inside a response, which V1 cannot express; it is deliberately not faked and belongs to the V3 adapter work.
- **Two things this gate must never do: refuse everything, or pretend to be dormant.** A caller that declares no scopes passes straight through, and so does one whose grant carries only names this build cannot map — failing closed on an unrecognised vocabulary would turn any upstream rename into a total outage. That is not hypothetical: the first version of this gate assumed *no* deployment sent the header, while the proxy had been forwarding `mcp:pro:read mcp:pro:write` on every `forward_identity` route all along, so every gated tool was refused on the closed endpoint until the two vocabularies were reconciled (see the correction under D3 and `CHANGELOG.md`). A malformed header still counts as "not declared" rather than "empty grant".
- **The refusal is in band** — an `isError` result, the same channel as the confirmation gate — never an HTTP 403, which drops the session, and never upstream API text. Hosts do not re-authorise on a mid-session denial (Claude Code closed that as "not planned", April 2026), so the message says not to retry and tells the model to route a named missing permission to a person.
...
**Addendum (2026-09-17) — destructive operations ask a human first.** `destructiveHint` is a hint the spec forbids clients from treating as a security boundary, and on an autonomous agent with no human in the loop it protects nothing at all. Elicitation is the protocol's own answer: the server asks, regardless of how the host is configured. Eleven tools are gated today (the eight `delete_*`/`unlink_*` tools, `remove_location_user`, `onboarding_rollback_phase` and the withheld `analytics_delete_assistant_report`).

- **Two paths, chosen from the capabilities the client declared at `initialize`.** With `elicitation`: an `elicitation/create` form naming the *resolved* object ("team member Ivan Petrov, Stylist, id 123") and the consequence; it runs only on an explicit accept, and a decline, a cancel or a failed prompt stops the call with no token issued. Without it: the first call performs nothing and returns the consequence plus a one-time `confirmation_token`, and the identical call repeated with that token proceeds. Without that fallback the call would simply hang on an older host.
- **The token is bound to the exact arguments.** HMAC over tool name + arguments, keyed per process, 10-minute TTL, refused rather than ignored when it does not verify — so a confirmation for client 5 never authorises deleting client 7. Nothing is stored server-side, which keeps `tools/list` static and no state session-scoped (D7); `confirmation_token` is an always-present optional argument, never negotiated per client.
- **Every non-approval outcome is an `isError` result** (confirmation required, declined, prompt undeliverable, invalid token), so a model cannot read "cancelled" as "deleted", and a gate result stays legal for a tool that declares an `outputSchema`.
- **Declarative, enforced in one place.** `confirm` on `ToolDefinition` adds both the schema argument and the gate registration; enforcement is the `tools/call` handler in `src/tools/registry.ts`, the only code path holding both the `Server` and the request id the prompt has to be related to.
...
5. ~~**Executor writes:** allow writes through `altegio_call_operation` at all, or curated tools only? Proposal: allowlist, with `destructiveHint` and explicit confirmation text.~~ **Resolved (2026-09-17): reads only, no allowlist.** The executor shipped read-only in the catalog work and stayed that way through four subsequent surface changes, which answered the question by use rather than by debate: every non-GET is refused with a pointer to the curated tool, and the policy is compile-time — `AltegioClient.request` is typed to the `'GET'` literal. Two things settled since the question was written also argue against reopening it. The confirmation gate (D8 addendum) attaches a human-readable consequence to *one named object*, which a generic "call this operation" tool cannot produce, so an allowlisted write would be the one destructive path with a worse prompt than the curated tools. And the scope map (D6 addendum) can only describe the executor coarsely — one tool name reaches every documented GET, so it is mapped to the `api:read` placeholder; adding writes would widen that single grant to cover write operations across every domain at once. The cost is a curated tool per write, which is the D2 bargain anyway. Revisit only when a scope can be resolved per catalog operation at call time, which needs the final v3 vocabulary first.
