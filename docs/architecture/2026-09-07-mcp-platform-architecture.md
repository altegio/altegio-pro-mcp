...
- **The names are placeholders.** Their shape follows the ratified `domain:action` convention and most are verbatim from the v3 P0 scope catalog, which is explicitly not final and is owned by the API team. `analytics:*` and `api:read` had to be invented: no v3 domain covers reports, and the universal executor reaches every documented GET under one tool name. Renaming them is expected, and cheap by construction.
...
- **A safe no-op today.** A caller that declares no scopes — the public HTTP endpoint, stdio, the closed Google deployment, all of them — passes straight through, and a malformed header counts as "not declared" rather than "empty grant", so a proxy bug cannot refuse every call. This is plumbing that starts enforcing by itself the day V3 issues narrowed tokens.
