# Temporary capacity, events, products and cash-flow reports
...
All four tools join analytics, finance, readonly and stdio; the default view stays
...
The capacity JSON chart returns daily percentages for a single member, not this
report's hour and day totals.
...
| Which inactive clients should we win back? | `analytics_get_client_reactivation_candidates` | Inclusive local-date cutoff, stable ids and prior engagement     |
...
All four legacy routes plus the client-base reactivation path are in the playbook; glossary/coverage and relevant review
